	...
	const slides = [
		{
			...
			description:
				'Explore vast product options. From tech to furniture, we have it all. Discover your perfect match.',
			image: consola,
		},
		{
			title: 'Checkout',
			description:
				'Select items and proceed to checkout. Credit check may apply. Get shipping details within 24 hours.',
			image: mobile,
		},
		{
			title: 'Enjoy your rental',
			description:
				'Experience the benefits of your rented items. Fast delivery to your doorstep for maximum enjoyment.',
			image: shipping,
		},
		{
			title: 'Sustainable Refurbishment',
			description:
				"Rent with us and experience convenience, flexibility, and resource conservation. Let's make a positive impact together!",
			image: cicle,
		},
	]
				...
				<section className='flex flex-col lg:flex-row items-center justify-center w-full'>
					<article className='h-60 w-full lg:w-1/2 bg-gray_light dark:bg-card_dark p-4 flex items-center justify-center order-2 lg:order-1'>
						<header className='text-center'>
							...
							<p className='text-base lg:text-lg leading-7 text-text_light dark:text-text_dark'>
								Explore placing an order, the advantages of renting, and our commitment to resource
								conservation.
							</p>
						</header>
					</article>
					...
					<article className='h-60 w-full lg:w-1/2 order-1 lg:order-2'>
						<img src={laptopYCajitas} alt='laptopCajira' className='h-full w-full object-cover ' />
					</article>
				</section>

				<section className='my-16'>
					<header className='text-center my-8'>
						<p className='text-text_gray font-bold text-md mb-5'>How it works</p>
						<h3 className='text-4xl font-bold text-medium_purple'>Getting started with Rent-ify</h3>
					</header>

					<article className='grid grid-rows-4 lg:grid-cols-2 lg:grid-rows-2 xl:grid-rows-1 xl:grid-cols-4 justify-center mt-8 space-y-8 md:space-y-0 md:space-x-1'>
						{slides.map((slide, index) => (
							<div
								key={index}
								className={`relative w-full transform transition-transform duration-500 ${
					...
					<div className='flex flex-col lg:flex-row justify-center mt-8 space-y-8 md:space-y-0 md:space-x-12'>
						...
						<div className='text-center flex flex-col items-center justify-between p-4'>
							...
							<p className='text-lg leading-6 text-text_light dark:text-text_dark'>
								...
								rental plan to match your changing needs.
							</p>
						</div>
