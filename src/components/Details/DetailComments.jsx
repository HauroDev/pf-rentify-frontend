...
const DetailComments = ({ idProd, commentes, star, average, updated, owner }) => {
	...
	const StarRating = ({ rating, onRatingChange }) => {
		return (
			<div>
				{[1, 2, 3, 4, 5].map((value) => (
					<Star key={value} filled={value <= rating} onClick={() => onRatingChange(value)} />
				))}
			</div>
		)
	}
			...
			<DetailCard>
				<div className='flex items-center mb-8 border-b-2 border-gray_medium pb-8'>
					<div className='flex flex-col'>
						<h2 className='text-3xl md:text-4xl text-text_light dark:text-text_dark'>
							Customer Review
						</h2>
								...
								{[5, 4, 3, 2, 1].map((num) => {
									...
									return (
										<div className='flex items-center mb-2' key={num}>
											<span className='text-gray-700 text-md font-semibold dark:text-text_dark'>
												{num}
											</span>
											<span className='text-yellow-400 mr-2 '>★</span>
											<div className='flex items-center h-4 w-64 bg-gray-300 rounded-lg overflow-hidden'>
												<div
													className='h-full bg-green_medium'
													style={{ width: `${progressWidth}%` }}></div>
											</div>
											<span className='ml-2 text-sm text-gray-600 dark:text-text_dark'>
												{star['s' + num]}
											</span>
										</div>
									)
				...
				<div>
					<div className='mb-4 '>
						<h2 className='text-text_light dark:text-text_dark text-2xl md:text-3xl'>
							Review the Product
						</h2>
					</div>
					<div>
					...
					<div className='mt-4 '>
						{userState.login && userState.user.idUser !== owner.idUser ? (
							<button
								className='bg-medium_purple hover:bg-dark_purple text-white px-4 py-2 rounded-lg text-lg'
				...
				<div>
					<h1 className='text-2xl md:text-3xl mt-4 mb-4 text-text_light dark:text-text_dark'>
						All reviews
					</h1>
								...
								<div>
									...
									<div className='flex items-center'>
										<div className='flex items-end'>
											...
											<span className='ml-2 text-gray_dark leading-6 text-sm'>
												{comment.user.name}
											</span>
										</div>
									</div>
									...
									<div className='col-span-1 flex items-start justify-end'>
										<p className='text-gray_dark leading-6 text-xs'>
											{`${new Date(comment.updatedAt).getDate().toString().padStart(2, '0')}/${(
												...
												.toString()
												.padStart(2, '0')}/${new Date(comment.updatedAt).getFullYear()}`}
										</p>
									</div>
								</div>
							)}
